...
def run(lock_file=LOCK_FILE, last_success_file=LAST_SUCCESS_FILE, fresh_threshold_seconds=FRESH_THRESHOLD_SECONDS):
	...
	try:
		...
		updateScheduleTracker(success=True, job_name="create", message="No-op: previous run still in flight")
		sys.exit(0)
	...
	if os.path.exists(last_success_file):
		...
		if age_seconds < fresh_threshold_seconds:
			...
			updateScheduleTracker(success=True, job_name="create", message="No-op: recent successful backup")
			sys.exit(0)
	...
	if failures:
		...
		updateScheduleTracker(success=False, job_name="create", message=summary)
	else:
		...
		with open(last_success_file, 'w') as f:
			f.write('')
		updateScheduleTracker(success=True, job_name="create")
