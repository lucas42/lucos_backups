...
- **Step zero (sysadmin):** verify aurora has rsync ≥3 and a hardlink-capable FS **at the actual backup-root path** (an SMB/NFS root silently degrades `--link-dest` to full copies — see §3); confirm the ProxyJump SSH path supports an rsync invocation (not just scp). This gates **all** implementation — rsync and restic are not a config flag apart (different code path, secret management, and restore runbook), so verification must complete *before any implementation begins*; if it fails, the work starts on the restic-over-SFTP fallback instead.
