...
Per lucas42's direction, the **planning and execution** of the actual ~85GB photos seed — staging it off-peak so it doesn't starve the nightly cron, and the verified-restore-before-import gate — is **not a standalone step in this ADR**. It is folded into the Google→lucOS migration plan in lucas42/lucos_photos#424, which owns the sequencing of the historical import. This ADR provides the *capability*; lucas42/lucos_photos#424 owns *when and how* the photos seed runs (coordinated with the architect, who holds the #424 design context).
...
- **Verified restore + seed sequencing (owned by lucas42/lucos_photos#424):** the end-to-end backup **and** restore of `photos` proving the mechanism, plus the off-peak seed run, are sequenced as part of the #424 migration plan and must complete **before** the 78GB import. This ADR provides the mechanism + `--seed` capability; #424 owns the run.
